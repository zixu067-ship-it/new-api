...
const ProfileForm = ({ initial, onSave, onCancel, mode = 'create' }) => {
  const [form, setForm] = useState({
    ...
    avatar: initial?.avatar_url || initial?.avatar || '',
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      // 头像同时用 avatar 和 avatar_url 两个字段名提交，兼容后端任意一个
      await onSave({
        real_name: form.name,
      ...
      <Card className="agent-glass agent-v3-content">
        ...
        {tab === 'settings' && (
          <div>
            ...
            <Card>
              <p><b>昵称：</b>{(profile?.real_name || profile?.nickname)}</p>
...
const GroupEditInline = ({ initial, onSave }) => {
  ...
  return (
    <Space vertical spacing="loose" style={{ width: '100%' }}>
      ...
      <div>
        <Text strong>对组员说的话</Text>
        <Input rows={3} value={f.message_to_members} onChange={(v) => setF({ ...f, message_to_members: v })} />
      </div>
...
const Agent = () => {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      // 后端字段可能是 leader_user_id / leader_id；类型可能 string/number
      const leaderId = Number(g.leader_user_id ?? g.leader_id ?? g.user_id ?? 0);
      const myId = Number(profileData.user_id ?? profileData.id ?? profileData.uid ?? 0);
      let isLeader = leaderId > 0 && leaderId === myId;

      // 兜底：直接试 members-profiles，能拿到说明你就是组长（接口本身有权限校验）
  ...
  return (
    <div className="agent-v3-root">
      <DebugPanel data={{ phase, profile, group, members, leader, leaderboard, _hint: 'leader_user_id 与 user_id 必须相等组长才显示为组长' }} />
