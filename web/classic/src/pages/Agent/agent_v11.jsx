...
const Agent = () => {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const leaderId = Number(g.leader_user_id ?? g.leader_id ?? 0);
      const myId = Number(profileData.user_id ?? profileData.id ?? 0);
      const isLeader = leaderId > 0 && leaderId === myId;
      if (isLeader) {
        const mr = await API.get('/api/agent/group/members-profiles').catch(() => null);
        if (mr && mr.data && mr.data.success) setMembers(mr.data.data || []);
        setPhase('leader');
      } else {
